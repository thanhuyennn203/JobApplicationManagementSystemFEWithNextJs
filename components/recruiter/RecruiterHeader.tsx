...
export default function RecruiterHeader() {
  ...
  useEffect(() => {
    const fetchPreference = async () => {
      try {
        ...
        const data = await getApplicationEmailNotificationPreference(
  ...
  const navItems = [
    { href: "/recruiter",             icon: "fa-house",            label: "Home" },
    { href: "/recruiter/packages",    icon: "fa-bag-shopping",     label: "Packages" },
    { href: "/recruiter/jobs",        icon: "fa-briefcase",        label: "Jobs" },
    { href: "/recruiter/application", icon: "fa-file-lines",       label: "Applications" },
    { href: "/recruiter/search",      icon: "fa-magnifying-glass", label: "Search" },
    { href: "/recruiter/company",     icon: "fa-building",         label: "Company" },
  ];
    ...
    <header className="sticky top-0 z-50 h-16 border-b border-slate-200 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex h-full max-w-[1600px] items-center justify-between px-6">
        ...
        <div className="flex items-center gap-10">
          <Link href="/recruiter/jobs" className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Hiring Dashboard</p>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={`
                    flex items-center gap-1 rounded-xl px-4 py-2 text-base font-medium transition-all
                    ${active
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            href="/recruiter/cart"
            className="relative flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-100"
              ...
              <div className="absolute right-0 mt-3 w-[340px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                ...
                <div className="border-b border-slate-100 p-4">
                  <div className="flex items-center gap-3">
                    <img src="/images/company-logo-default.jpg" className="h-12 w-12 rounded-full" />
                    <div>
                      <p className="font-semibold text-slate-800">Recruiter Account</p>
                      <p className="text-sm text-slate-500">Manage your hiring activity</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-100 p-4">
                  ...
                  <div className="space-y-3">
                    <label className="flex cursor-pointer items-start justify-between rounded-xl border border-slate-200 p-3 transition hover:border-slate-300 hover:bg-slate-50">
                      <div>
                        <p className="text-sm font-semibold text-slate-800">Daily at 9:00 AM</p>
                        <p className="mt-1 text-xs text-slate-500">Receive daily hiring summary email</p>
                      </div>
                      <input
                        type="radio"
                        checked={emailPreference === ApplicationEmailNotificationPreference.DAILY}
                        onChange={() => handleUpdateEmailPreference(ApplicationEmailNotificationPreference.DAILY)}
                    ...
                    <label className="flex cursor-pointer items-start justify-between rounded-xl border border-slate-200 p-3 transition hover:border-slate-300 hover:bg-slate-50">
                      <div>
                        <p className="text-sm font-semibold text-slate-800">Each applicant</p>
                        <p className="mt-1 text-xs text-slate-500">Get instant email when someone applies</p>
                      </div>
                      <input
                        type="radio"
                        checked={emailPreference === ApplicationEmailNotificationPreference.EACH_APPLICATION}
                        onChange={() => handleUpdateEmailPreference(ApplicationEmailNotificationPreference.EACH_APPLICATION)}
                        className="mt-1 h-4 w-4"
                      />
                    </label>
                    ...
                    <label className="flex cursor-pointer items-start justify-between rounded-xl border border-slate-200 p-3 transition hover:border-slate-300 hover:bg-slate-50">
                      <div>
                        <p className="text-sm font-semibold text-slate-800">No email</p>
                        <p className="mt-1 text-xs text-slate-500">Disable all application emails</p>
                      </div>
                      <input
                        type="radio"
                        checked={emailPreference === ApplicationEmailNotificationPreference.NONE}
                        onChange={() => handleUpdateEmailPreference(ApplicationEmailNotificationPreference.NONE)}
                        className="mt-1 h-4 w-4"
                      />
                    </label>
                  </div>
